...
static bool is_amd_pmu_msr(const u32 msr)
{
	if (boot_cpu_data.x86_vendor != X86_VENDOR_AMD &&
...
static bool is_intel_pmu_msr(const u32 msr_index, int *type, int *index)
{
	u32 msr_index_pmc;
...
static bool xen_intel_pmu_emulate(const u32 msr, u64 *val, int type, int index, bool is_read)
{
	uint64_t *reg = NULL;
...
bool pmu_msr_chk_emulated(const u32 msr, u64 *val, bool is_read, bool *emul)
{
	int type, index = 0;
