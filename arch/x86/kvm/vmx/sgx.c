...
void setup_default_sgx_lepubkeyhash(void)
{
	...
	if (!enable_sgx || boot_cpu_has(X86_FEATURE_SGX_LC) ||
	    native_rdmsrq_safe(MSR_IA32_SGXLEPUBKEYHASH0, &sgx_pubkey_hash[0])) {
		...
		sgx_pubkey_hash[1] = native_rdmsrq(MSR_IA32_SGXLEPUBKEYHASH1);
		sgx_pubkey_hash[2] = native_rdmsrq(MSR_IA32_SGXLEPUBKEYHASH2);
		sgx_pubkey_hash[3] = native_rdmsrq(MSR_IA32_SGXLEPUBKEYHASH3);
	}
}
